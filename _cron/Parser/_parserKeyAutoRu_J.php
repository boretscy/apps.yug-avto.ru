...
*/
	...
	$arAds = $app->Parser->getAdsAutoKey('J');
	...
	foreach ($arAds as $Ad) {
		...
		if (!$curAd) {
			...
			if ($curAd['price'] != $Ad['price']) {
				...
				$app->db->query('INSERT INTO parser_history_key SET ?u', $arIns);
	...
	if (count($newAds)>0 || count($changeAds)>0) $app->sendParserNotify($newAds, $changeAds, 'AUTO.RU КЛЮЧАВТО', 'Jaguar');
?>