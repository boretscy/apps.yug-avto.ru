 ...
 * @license GPLv2
 *
 * Fork https://github.com/Clorith/SQLSRV-DataBase-Class
 * init params is array
 */
...
class SafeMSSQL {
	...
	public function __construct( $opt = [] ) {
		...
		$this->DBConf = (object)array_merge( $this->defaults, $opt );
		
		$this->is_connected = $this->db_connect();

		// If we've chosen to build a database schema, this is done on construct
		if ( $this->is_connected && $this->DBConf->build_schema ) {
			
			if ( is_string( $this->DBConf->build_schema ) ) {
				
				$this->schema_location = $this->DBConf->build_schema;
			...
			$this->schema = $this->DBConf->build_schema();
		}
	}
	...
	public function db_connect() {
		$serverName = "tcp:" . $this->DBConf->host . ", " . $this->DBConf->port;
		$connectionOptions = array(
			"Database" => $this->DBConf->db,
			"UID"      => $this->DBConf->user,
			"PWD"      => $this->DBConf->pass
		);
