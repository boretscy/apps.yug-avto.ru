    ...
    <div class="row">
      <div class="col-md-12">
         ...
         <?php
		  ...
		  if ( $_GET ) $arFilter['export'] = '/hot/export/?'.http_build_query($_GET);
        ...
        <?php HTML::statAppFilter( $arFilter ); ?>
         ...
         <div class="box box-primary">
        	...
        	<div class="box-header with-border"><h3 class="box-title">Активность пользователей приложения "<?=$app->Hot->AppInfo()->ru_name?>" <small>(<?=count($arStat)?> <?=Helper::getWorld(count($arStat), 'record')?>)</small></h3></div>
              ...
              <table id="data-table-stats" class="table table-hover table-striped table-condensed dataTable">
                <thead>
                  <tr>
                    ...
                    <th style="width: 20%">Автомобиль</th>
                ...
                <tbody>
                  ...
                  <?php
				  	$res = $app->Hot->getItem( $item['item_id'] );
				  ?>
                  ...
                  <tr>
                    ...
                    <td><?=$item['dc']?></td>
                    <td><?=$item['model']?>, <?=$item['complectation']?>, <?=$item['gearbox']?>, <?=$item['year']?></td>
                    <td><?=$res['vin']?></td>
                    <td><?=$item['name']?></td>
