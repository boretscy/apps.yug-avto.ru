<?php
	...
	if ( $_GET['hide_doubles'] == 'on' ) {
		...
		foreach ( $arStat as $k => $r ) {
			...
			if ( $r['site_id']==$arStat[$k+1]['site_id'] && $r['name']==$arStat[$k+1]['name'] && $r['phone']==$arStat[$k+1]['phone'] && $r['email']==$arStat[$k+1]['email'] ) unset( $arStat[$k] );
		}
	}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1><?=$app->Chat->AppInfo()->ru_name?> <small><?=HTML::getPageTitle( $currentRoute )?></small></h1>
  </section>
  
  <!-- Main content -->
  <section class="content">
    
    <?php if ( $app->Chat->AppInfo()->maintenance ) include $_SERVER['DOCUMENT_ROOT'].'/upload/Apps/maintenance.php'; ?>
    ...
    <div class="row">
      <div class="col-md-12">
		  ...
		  if ( $_GET ) $arFilter['export'] = '/chat/export/?'.http_build_query($_GET);
        ...
        <?php HTML::statAppFilter( $arFilter ); ?>
         ...
         <div class="box box-primary">
        	...
        	<div class="box-header with-border"><h3 class="box-title">Активность пользователей приложения "<?=$app->Chat->AppInfo()->ru_name?>" <small>(<?=count($arStat)?> <?=Helper::getWorld(count($arStat), 'record')?>)</small></h3></div>
              ...
              <table id="data-table-stats" class="table table-hover table-striped table-condensed dataTable">
                <thead>
                  <tr>
					...
					<th style="width: 30%">Данные посетителя<br /><small>Имя / Телефон / Email</small></th>
                ...
                <tbody>
                  ...
                  <tr>
					...
					<td>
						<?=(($item['name'])?:'')?><?=(($item['name'])?' / ':'')?>
						<?=(($item['phone'])?Helper::formatPhoneOut($item['phone']):'')?><?=(($item['email'])?' / ':'')?>
						<?=(($item['email'])?:'')?>
					</td>
                    ...
                    <td class="text-right">
                      ...
                      <a href="<?=$item['source_url']?>" target="_blank" class="span-label hint--top" aria-label="<?=$item['source_title']?>"><span class="label label-default"><i class="fa fa-external-link" aria-hidden="true"></i></span></a>
                    </td>
                  </tr>
                  <? } ?>
                </tbody>
