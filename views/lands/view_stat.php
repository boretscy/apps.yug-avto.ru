<?php
	...
	if ( $_GET['hide_doubles'] == 'on' ) {
		...
		foreach ( $arStat as $k => $r ) {
			...
			if ( $r['land_id']==$arStat[$k+1]['land_id'] && $r['name']==$arStat[$k+1]['name'] && $r['phone']==$arStat[$k+1]['phone'] && $r['form']==$arStat[$k+1]['form'] ) unset( $arStat[$k] );
		}
	}
?>
            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-header with-border">
                        <h3 class="box-title">Активность пользователей приложения "<?=$app->Lands->AppInfo()->ru_name?>" <small>(<?=count($arStat)?> <?=Helper::getWorld(count($arStat), 'record')?>)</small></h3>
                    </div>
                    ...
                    <div class="box-body">
                        ...
                        <table id="data-table-stats" class="table table-hover table-striped table-condensed dataTable">
                            <thead>
                                ...
                                <th style="width: 15%">Посадочная страница</th>
                                <th style="width: 15%">Имя</th>
                                <th style="width: 15%">Телефон</th>
                                <th style="width: 20%">Форма, ДЦ</th>
                            ...
                            <tbody>
                                ...
                                <td><?=$app->Lands->getLand($item['land_id'])['ru_name']?></td>
                                <td><?=$item['name']?></td>
                                <td><?=(($item['phone'])?Helper::formatPhoneOut($item['phone']):'<small>---</small>')?></td>
                                <td>
                                    <?=$item['form']?>
                                    <?= (($item['dc_id'])?'<br />'.$app->getDC($item['dc_id'])['ru_name']:'');?>
                                </td>
